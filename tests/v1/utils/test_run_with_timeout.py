...
class TestOperationManager:
    ...
    def test_successful_operation(self):
        ...
        result = manager.run_with_timeout(
            successful_func, timeout_seconds=1.0, label="test_op"
        )
    ...
    def test_failure_count_increments(self):
        ...
        for i in range(3):
            with pytest.raises(OperationTimeoutError):
                manager.run_with_timeout(
                    timeout_func, timeout_seconds=0.1, label=f"op_{i}"
                )
    ...
    def test_operations_continue_after_timeouts(self):
        ...
        for i in range(5):
            with pytest.raises(OperationTimeoutError):
                manager.run_with_timeout(
                    timeout_func, timeout_seconds=0.1, label=f"timeout_{i}"
                )
    ...
    def test_exception_propagation(self):
        ...
        with pytest.raises(ValueError) as exc_info:
            manager.run_with_timeout(
                raising_func, timeout_seconds=1.0, label="raising_op"
            )
