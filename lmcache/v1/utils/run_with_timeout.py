...
class OperationManagerStats:
    ...
    def get_summary_line(self) -> str:
        ...
        for label, stats in self.operation_stats.items():
            op_summaries.append(f"{label}={stats.timeout_count}/{stats.total_count}to")
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "operations": {
                label: stats.to_dict() for label, stats in self.operation_stats.items()
            },
        }
...
class OperationManager:
    ...
    def __init__(self, num_threads: int = 4, logger: Optional[logging.Logger] = None):
